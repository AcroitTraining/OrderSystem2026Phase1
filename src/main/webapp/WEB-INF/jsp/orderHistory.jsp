			...
			<c:choose>
				...
				<c:when test="${empty orderHistoryList}">
					<div style="padding: 10px 10px; text-align: center;">
						<div
							style="background: #FFD700; padding: 110px; border-radius: 20px; font-size: 0.8rem; font-weight: bold;">
							注文履歴がありません</div>
					</div>
				</c:when>
				<%-- 注文している商品がある時表示 --%>
					...
					<table class="order-table">
						...
						<tbody>
							<c:forEach var="item" items="${orderHistoryList}">
								<tr>
									<td><strong>${item.productName}</strong><br> <c:forEach
											var="topping" items="${item.toppings}">
											<span style="color: #666; font-size: 0.9rem;">・${topping.name}✕${topping.quantity}</span>
											<br>
											<!-- 各トッピング名と各トッピング個数表示 -->
										</c:forEach></td>
				...
				<c:if test="${not empty orderHistoryList}">
					<form action="OrderHistoryServlet" method="post"
						...
						<button type="submit" name="action" value="checkOut"
							class="btn-checkout">
							<div
								style="border: 2px solid black; border-radius: 50%; width: 30px; height: 30px; margin: 0 auto 5px; display: flex; align-items: center; justify-content: center;">￥</div>
							<strong>お会計</strong>
						</button>
					</form>
				</c:if>
			...
			<div class="modal-content">
				<c:choose>
					...
					<c:when test="${popupStatus == 1}">
						...
						<div class="btn-group">
							<button type="button" class="btn-base btn-close"
					...
					<c:when test="${popupStatus == 2}">
						...
						<form action="OrderHistoryServlet" method="post" class="btn-group">
							...
							<input type="hidden" name="totalOrderPrice"
								value="${totalOrderPrice}">
							<button type="button" class="btn-base btn-no"
								onclick="closeModal()">いいえ</button>
							<button type="submit" name="action" value="yes"
								class="btn-base btn-yes">はい</button>
						</form>
					</c:when>
				</c:choose>
			</div>
